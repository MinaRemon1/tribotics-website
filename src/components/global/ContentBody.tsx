    ...
    <div className="relative w-full flex items-center justify-center">
        <BackgroundGradientAnimation2>
          <div className="absolute z-50 inset-0 flex flex-col items-start pt-[10%] pl-[10%] pb-[2%] justify-center text-zinc-500 font-bold px-4 pointer-events-none text-left space-y-4">
            <div>
              <Heading className="text-[1.5rem] md:text-[1.5rem] lg:text-[2rem] text-slate-200 pr-[5%] md:pr-[10%] lg:pr-[30%]">{page.data.title}</Heading>
              {/* <div className="flex gap-4 text-purple-600 text-xl font-bold">
                {page.tags.map((tag)=>(
                  <span key={tag}>{tag}</span>
                ))}
              </div> */}
              <p className="flex gap-4 mt-8 text-md md:text-lg lg:text-lg pl-[7%] text-left font-medium text-slate-800">
                {formattedDate}
              </p>
            </div>
          </div>
